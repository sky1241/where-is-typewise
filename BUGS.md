...
- **How it was found**: Sky's screenshot of the deployed Streamlit Cloud dashboard after the real DB was pushed. Same pattern as BUG-001 — pytest can't catch what the user-facing surface silently misrepresents. **Lesson: every change that touches a metric needs a visual or live-API verification, not just tests.**
...
- **How it was found**: visual inspection of a Playwright screenshot during the "no silent bugs" audit Sky requested. **Lesson: green tests are not a substitute for actually running the user-facing surface and looking at it.**
