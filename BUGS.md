...
- **Fix**: dropped the time window on the dashboard headline metrics — pass `since_days=None`. Added a `_count_mentioned(conn)` helper that counts across the whole indexed dataset. Updated sub-labels from "threads this week" to "threads in the radar" / "across the indexed dataset" to stay honest about what the number reflects. `_count_mentioned_recent` kept as a windowed variant for future digest-style callers.
- **Test**: `tests/test_app.py::test_headline_counts_old_hot_threads_without_time_window` reproduces the failure shape (hot threads older than any 7-day window must still be counted when `since_days=None`); `tests/test_radar_store.py::test_count_unmentioned_relevant_matches_acceptance_query` covers the store-level `since_days=None` path. Original verification was a manual visual smoke at `http://localhost:8501` (`Metrics: ['5', '0', '5:0']`).
